...
- [x] **HA integrations silently broken (pre-existing)** — unused `tautulli` and `wyoming` config entries deleted via the HA UI (they were never in `extraComponents`, so they failed to import at every start).
